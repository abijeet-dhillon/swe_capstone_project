...
def test_storage_delete_all_zip_and_projects_counts():
    with tempfile.TemporaryDirectory() as td:
        ...
        with sqlite3.connect(s.db_path) as conn:
            z_before = conn.execute("SELECT COUNT(1) FROM ingest_sources").fetchone()[0]
            p_before = conn.execute("SELECT COUNT(1) FROM project_runs").fetchone()[0]
        ...
        with sqlite3.connect(s.db_path) as conn:
            z_after = conn.execute("SELECT COUNT(1) FROM ingest_sources").fetchone()[0]
            p_after = conn.execute("SELECT COUNT(1) FROM project_runs").fetchone()[0]
